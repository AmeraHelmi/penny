...
add_action('wp_ajax_autosuggest_it', 				'PennyTheme_autosuggest_it');
add_action('wp_ajax_nopriv_autosuggest_it', 		'PennyTheme_autosuggest_it');

function PennyTheme_autosuggest_it()
{
	...
	$new_string = substr ( $new_string,0, ( strLen ( $new_string ) -1 ) );
		...
		if ( strlen ( $new_string ) > 0 ):
			...
			if($total>0):
					...
					while ( $row = mysql_fetch_assoc ( $query ) ): 
						...
						$prm = get_permalink($row['ID']);			
								...
								src="'.PennyTheme_get_first_post_image($row['ID'],36,36).'" /></h2> <p>'.$row['post_title'].'</p></li>';
						echo '</ul>';			
					endwhile;
			else:
			...
			echo '<ul>';				
	         		echo '<li onClick="fill(\''.$new_string.'\');">'.__('No results found','PennyTheme').'</li>';
	        echo '</ul>';

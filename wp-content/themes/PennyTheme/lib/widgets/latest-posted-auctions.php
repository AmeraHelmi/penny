...
function register_latest_posted_auctions_widget() {
	register_widget('PennyTheme_latest_posted_auctions');
}

class PennyTheme_latest_posted_auctions extends WP_Widget {

	function PennyTheme_latest_posted_auctions() {
		$widget_ops = array( 'classname' => 'latest-posted-auctions', 'description' => 'Show latest posted auctions' );
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'latest-posted-auctions' );
		$this->WP_Widget( 'latest-posted-auctions', 'PennyTheme - Latest Auctions', $widget_ops, $control_ops );
	}

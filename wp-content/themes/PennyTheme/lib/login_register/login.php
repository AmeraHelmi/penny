...
function PennyTheme_sitemile_filter_ttl($title){ global $real_ttl; return $real_ttl." - ".get_bloginfo('sitename');}

function PennyTheme_do_login_scr()
		{
		...
		<?php
			...
			default:
			  ...
			  //Any differences will be noted with end of line comments. 
				 ...
				 */
				 if ( !isset( $_REQUEST['redirect_to'] ) ) {
					$redirect_to = get_permalink(get_option('PennyTheme_my_account_page_id'));
				 } else {
					$redirect_to = $_REQUEST['redirect_to'];
				 }
				 
				 if(empty($redirect_to)) $redirect_to = get_permalink(get_option('PennyTheme_my_account_page_id'));
		...
		?>
            ...
            <div class="padd10">
                ...
                &nbsp;&nbsp;&nbsp;or don't have an account <a style="color: white;text-decoration: none;padding: 7px 8px;" href="<?php bloginfo('siteurl') ?>/wp-login.php?action=register" class="submit_bottom">Register</a>
